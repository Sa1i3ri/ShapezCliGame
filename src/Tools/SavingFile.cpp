...
void SavingFile::readLevel(const vector<vector<string>>& file,Level* level) {
    ...
    //读取开采矿物数量
    level->map->AMineralNum = stoi(file[levelIndex-1][1]);
    level->map->BMineralNum = stoi(file[levelIndex-1][2]);
    level->map->HalfAMineralNum = stoi(file[levelIndex-1][3]);
    //器械速度
    level->map->ExtractorInterval = stoi(file[levelIndex-1][4]);
    level->map->TransmissionBeltInterval = stoi(file[levelIndex-1][5]);
    level->map->CutterInterval = stoi(file[levelIndex-1][6]);
    level->map->CentreInterval = stoi(file[levelIndex-1][7]);
    level->map->RubbishBinInterval = stoi(file[levelIndex-1][8]);
    level->map->upGradeTimes = stoi(file[levelIndex-1][9]);
    //任务完成情况
    level->map->mission1 = stoi(file[levelIndex-1][10]);
    level->map->mission2 = stoi(file[levelIndex-1][11]);
    level->map->mission3 = stoi(file[levelIndex-1][12]);
...
vector<vector<string>> SavingFile::writeLevel(Level* level) {
    ...
    //开采的矿物数量
    line.push_back(to_string(level->map->AMineralNum));
    line.push_back(to_string(level->map->BMineralNum));
    line.push_back(to_string(level->map->HalfAMineralNum));
    //器械速度
    line.push_back(to_string(level->map->ExtractorInterval));
    line.push_back(to_string(level->map->TransmissionBeltInterval));
    line.push_back(to_string(level->map->CutterInterval));
    line.push_back(to_string(level->map->CentreInterval));
    line.push_back(to_string(level->map->RubbishBinInterval));
    //升级次数
    line.push_back(to_string(level->map->upGradeTimes));
    //任务完成情况
    line.push_back(to_string(level->map->mission1));
    line.push_back(to_string(level->map->mission2));
    line.push_back(to_string(level->map->mission3));
